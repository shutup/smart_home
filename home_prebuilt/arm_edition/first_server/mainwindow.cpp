...
int MainWindow::Ser_cmd_parser(QString cmd)
{
    ...
    else if(cmd.startsWith(Utils::SET_HUMI_MIN))
    {
        ...
        //
        bool flag=Utils::update_value_min(Utils::HUMI_ID,QString::number(value));
//        boolean flag=Utils::update_value_min(Utils::HUMI_ID,cmd.right(offset));
    ...
    else if(cmd.startsWith(Utils::SET_HUMI_MAX))
    {
        ...
        bool flag=Utils::update_value_max(Utils::HUMI_ID,QString::number(value));
//        boolean flag=Utils::update_value_max(Utils::HUMI_ID,cmd.right(offset));
    ...
    else if(cmd.startsWith(Utils::SET_TEMP_MIN))
    {
        ...
        //
        bool flag=Utils::update_value_min(Utils::TEMP_ID,QString::number(value));
//        boolean flag=Utils::update_value_min(Utils::TEMP_ID,cmd.right(offset));
    ...
    else if(cmd.startsWith(Utils::SET_TEMP_MAX))
    {
        ...
        //
        bool flag=Utils::update_value_max(Utils::TEMP_ID,QString::number(value));
    ...
    //定义包含串口配置信息的串口配置类对象
        ...
        for(int i=0;i<30;i++)
        {
            ...
            if(flag==false)
            {
                ...
                //初始化串口对象
                //输入：串口名，串口配置信息，查询模式
                ser_Com=new Posix_QextSerialPort(serial_name,serComSetting,QextSerialBase::Polling);
                //调试信息：输出当前正在测试的串口名
                // qDebug()<<serial_name;
                //尝试打开串口设备
